...
query = """
  SELECT 
    ...
    SUM(
      CASE
        WHEN s2.descricao = 'EXAGERADO' THEN OP.VALORORIGINAL * OP.QUANTIDADE
        WHEN f.TOTAL IS NOT NULL AND s2.descricao <> 'EXAGERADO'THEN op.VALORORIGINAL * OP.QUANTIDADE - f.TOTAL 
        ELSE op.VALORORIGINAL * OP.QUANTIDADE
      END
    ) AS TOTAL
